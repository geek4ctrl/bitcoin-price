...
class ChartBar extends Component {
  ...
  dataPlotClick(eventObj, dataObj) {
    ...
    function displayPrimeOf(originalNumber) {
      ...
      for (let number = 2; number <= originalNumber; number++)
        if (checkPrimeOf(number)) {
          totalPrime++;
        }
      return totalPrime;
    }

    function checkPrimeOf(number) {
    ...
    this.setState({
      message: [
        ...
        ". The prime number is: ",
        <strong>{dataObj.displayValue}</strong>,
        ". The total number of prime number is: ",
        <strong>{countOfPrimeNumbers}</strong>,
        ". Is the number of prime a prime? ",
        <strong>{isNumberOfPrimeAprime.toString()}</strong>
      ]
    });
  }
  ...
  render() {
    ...
    for (var i = 0; i < this.props.items[0].labels.length; i++) {
      objs.push(
        new Obj(
          this.props.items[0].labels[i],
          this.props.items[0].datasets[0].data[i]
        )
      );
    }
    ...
    return (
      <div>
        ...
        <div style={{ padding: "5px" }} id="message">
          {this.state.message ||
            "Click the below buttons to add an event dynamically to a charts"}
        </div>
    ...
    return (
      <div className="App">
        <div className="title">
          {" "}
          <h1>
            Bit<span className="coin">coin</span> market
          </h1>{" "}
        </div>
        ...
        {this.state.firstDate && this.state.secondDate && (
          <div className="period">
            <label data-param="1" onClick={this.handleDays}>
              Search
            </label>
          </div>
        )}
